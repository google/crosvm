...
mod tests {
    ...
    impl VhostBackend for MockBackend {
        ...
        fn set_mem_table(&self, _regions: &[VhostUserMemoryRegionInfo]) -> Result<()> {
            Ok(())
        }
        ...
        fn set_vring_addr(&self, queue_index: usize, _config_data: &VringConfigData) -> Result<()> {
            assert_eq!(queue_index, 1);
            Ok(())
        }
        ...
        fn set_vring_call(&self, queue_index: usize, _event: &Event) -> Result<()> {
            assert_eq!(queue_index, 1);
            Ok(())
        }

        fn set_vring_kick(&self, queue_index: usize, _event: &Event) -> Result<()> {
            assert_eq!(queue_index, 1);
            Ok(())
        }

        fn set_vring_err(&self, queue_index: usize, _event: &Event) -> Result<()> {
            assert_eq!(queue_index, 1);
            Ok(())
        }
        ...
        fn restore(&self, _data_bytes: &[u8], _queue_evts: Option<Vec<Event>>) -> Result<()> {
            Ok(())
        }
    }

    #[test]
    fn test_vring_backend_mock() {
        let b = MockBackend {};
