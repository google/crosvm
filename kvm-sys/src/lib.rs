...
ioctl_ior_nr!(KVM_GET_MSR_INDEX_LIST, 0x02, kvm_msr_list);
ioctl_io_nr!(KVM_CHECK_EXTENSION, 0x03);
ioctl_io_nr!(KVM_GET_VCPU_MMAP_SIZE, 0x04) /* in bytes */;
ioctl_ior_nr!(KVM_GET_SUPPORTED_CPUID, 0x05, kvm_cpuid2);
ioctl_ior_nr!(KVM_GET_EMULATED_CPUID, 0x09, kvm_cpuid2);
...
ioctl_ior_nr!(KVM_GET_IRQCHIP, 0x62, kvm_irqchip);
ioctl_ior_nr!(KVM_SET_IRQCHIP, 0x63, kvm_irqchip);
ioctl_io_nr!(KVM_CREATE_PIT, 0x64);
ioctl_ior_nr!(KVM_GET_PIT, 0x65, kvm_pit_state);
ioctl_ior_nr!(KVM_SET_PIT, 0x66, kvm_pit_state);
ioctl_ior_nr!(KVM_IRQ_LINE_STATUS, 0x67, kvm_irq_level);
...
ioctl_ior_nr!(KVM_CREATE_DEVICE, 0xe0, kvm_create_device);
...
ioctl_ior_nr!(KVM_TRANSLATE, 0x85, kvm_translation);
ioctl_iow_nr!(KVM_INTERRUPT, 0x86, kvm_interrupt);
ioctl_ior_nr!(KVM_GET_MSRS, 0x88, kvm_msrs);
...
ioctl_ior_nr!(KVM_GET_CPUID2, 0x91, kvm_cpuid2);
ioctl_ior_nr!(KVM_TPR_ACCESS_REPORTING, 0x92, kvm_tpr_access_ctl);
...
ioctl_iow_nr!(KVM_SET_XCRS, 0xa7, kvm_xcrs);
...
ioctl_ior_nr!(KVM_GET_REG_LIST, 0xb0, kvm_reg_list);
