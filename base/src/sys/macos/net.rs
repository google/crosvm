...
impl UnixSeqpacket {
    ...
    /// so we use `SOCK_STREAM` instead. This works because the tube
    /// protocol (in `sock_ctrl_msg`) sends a fixed-size header followed
    /// by data, which provides its own framing on top of the byte stream.
    /// Note that `SOCK_STREAM` does not preserve message boundaries, so
    /// the framing in the tube layer is required for correctness.
    ///
