...
///
/// Returns [dist_base, dist_size, redist_base, total_redist_size] on success.
fn initialize_hvf_gic(num_vcpus: usize) -> std::result::Result<[u64; 4], String> {
    ...
    // Layout: [distributor][redistributors][0x40000000]
    // HVF requires redistributors at higher addresses than the distributor.
    ...
    Ok([dist_base, dist_size, redist_base, total_redist_size])
}
...
pub struct HvfIrqChip {
    ...
    /// GIC properties [dist_base, dist_size, redist_base, total_redist_size].
    gic_props: Option<[u64; 4]>,
}
...
impl HvfIrqChip {
    ...
    pub fn new(num_vcpus: usize) -> Result<HvfIrqChip> {
        let (gic_initialized, gic_props) = match initialize_hvf_gic(num_vcpus) {
            Ok(props) => (true, Some(props)),
            Err(e) => {
                error!(
...
impl IrqChip for HvfIrqChip {
    ...
    fn register_edge_irq_event(
        ...
        info!("register_edge_irq_event: gsi={} index={} device={}", irq, index, source.device_name);
    ...
    fn service_irq_event(&mut self, event_index: IrqEventIndex) -> Result<()> {
        ...
        if let Some(evt) = events.get(event_index) {
            ...
            if self.gic_initialized {
                info!("service_irq_event: index={} gsi={} device={}", event_index, evt.gsi, evt.source.device_name);
                hvf_gic_set_spi(evt.gsi, true);
                hvf_gic_set_spi(evt.gsi, false);
            } else {
                error!("service_irq_event: GIC not initialized, dropping gsi={}", evt.gsi);
            }
