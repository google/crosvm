...
impl VioSClient {
    ...
    fn request_info<T: Default + Copy + Clone>(
        &self,
        req_code: u32,
        count: usize,
    ) -> Result<Vec<T>> {
        ...
        Ok(reply[status_size..]
            ...
            .map(|info_buffer| {
                // SAFETY:
                // `info_buffer` is guaranteed to be `info_size` bytes long by `chunks()`.
                // `info_size` is exactly `std::mem::size_of::<T>()`.
                // We use `read_unaligned` because the byte slice may not be properly aligned for
                // `T`.
                unsafe { std::ptr::read_unaligned(info_buffer.as_ptr() as *const T) }
            })
            .collect())
    }
